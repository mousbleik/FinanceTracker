// Proposal B — application logic (identical behaviour to Proposal A; only the
// presentation/stack differs). Looks up an attendee from window.ATTENDEES,
// renders the badge and drives printing. No Google Apps Script dependency.
...
(function () {
  ...
  function qrSrc(text) {
    return "https://api.qrserver.com/v1/create-qr-code/?size=110x110&margin=4&data=" +
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (att) {
      ...
      msg.className = "text-sm mt-3 min-h-[1.2em] is-ok";
      render(att);
    } else {
      msg.textContent = "No attendee found for “" + input.value.trim() + "”.";
      msg.className = "text-sm mt-3 min-h-[1.2em] is-error";
      clearResult();
    }
  });
